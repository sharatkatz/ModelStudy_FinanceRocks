...
class ModelTrain(PrepareInDatForModeling, PreProcessor):
    ...
    def setup_OLS_with_CV(self, cv_folds: int = 5, scoring: str = 'r2', random_state: int = 42):
        """
        Configure and run OLS regression with k-fold cross-validation.

        This method extends the original OLS setup with cross-validation to provide
        more robust performance estimates and prevent overfitting.

        Workflow
        --------
        1. Uses the same data preparation pipeline as setup_OLS()
        2. Performs k-fold cross-validation to estimate model performance
        3. Provides cross-validation scores and stability analysis
        4. Compares training vs validation performance
        ...
        ols_train_test_gap : float
            Difference between training and validation performance
        feature_stability : pandas.Series
            Frequency of feature selection across CV folds

        Examples
        --------
        >>> analyzer = YourClass()
        >>> analyzer.setup_OLS_with_CV(cv_folds=5)
        >>> print(f"CV R²: {analyzer.ols_cv_mean:.4f} (+/- {analyzer.ols_cv_std * 2:.4f})")
        """
        try:
            # Use existing OLS data preparation
            if not hasattr(self, 'mod_dsn') or not hasattr(self, 'exog_glm'):
                self.logger.info("Running original OLS setup first...")
                self.setup_OLS()
            ...
            self._perform_ols_cross_validation(X, y, cv_folds, scoring, random_state)

            # Analyze feature stability across folds
            self._analyze_feature_stability(X, y, cv_folds, random_state)

            # Compare with original OLS results
            self._compare_cv_with_original_ols()

            # Check for overfitting
            self._assess_overfitting()
    ...
    def _perform_ols_cross_validation(self, X, y, cv_folds, scoring, random_state):
        ...
        # Create OLS model (without intercept since your data likely already has one)
        ...
        self.logger.info("=" * 50)
        self.logger.info("OLS CROSS-VALIDATION RESULTS")
        self.logger.info("=" * 50)
    ...
    def _analyze_feature_stability(self, X, y, cv_folds, random_state):
        ...
        feature_importance['cv'] = (feature_importance['std_coef'] / feature_importance['mean_coef'].abs()).replace([np.inf, -np.inf], np.nan).fillna(0)

        # Frequency of non-zero coefficients (for comparison with regularized methods)
    ...
    def _compare_cv_with_original_ols(self):
        """Compare CV OLS results with original OLS results."""
        if not hasattr(self, 'final_ols_results'):
            self.logger.warning("Original OLS results not available for comparison")
            return

        # Compare coefficients
        original_coefs = pd.Series(self.final_ols_results.params, name='original_ols')
        cv_coefs = pd.Series(self.final_ols_cv_model.coef_, index=self.exog_glm, name='cv_ols')
        ...
        self.logger.info(f"Original OLS R²: {self.final_ols_results.rsquared:.4f}")
        self.logger.info(f"CV OLS R²: {r2_score(self.y_ols, self.final_ols_cv_predictions):.4f}")
        self.logger.info(f"Largest coefficient differences:")
    ...
    def _assess_overfitting(self):
        ...
        if abs(train_cv_gap) > 0.1:
            self.logger.warning("Large train-CV gap detected - potential overfitting!")
    ...
    def undummify_company_type_labels(
        ...
        self.scatter_resid_with_predictors(outdf, 'company_type', self.final_ols_results, resid=None, model_type="OLS")
        return outdf
    ...
    def model_diagnostics(
            ...
            model_results: Union[sm.OLS, sm.GLS],
            model_type: str) -> None:
        """
        ...
        performance_cols: List[str] = list(self.map_ordinal_variables_and_category_order().keys())
        ...
        # Calculate residuals if not present in model_results (e.g. for LASSO)
        residuals = None
        if hasattr(model_results, 'resid'):
            residuals = model_results.resid
        else:
            # For LASSO/RIDGE, we need to calculate residuals manually
            # Note: LASSO model was trained on standardized data
            self.logger.info("Calculating residuals manually for LASSO/RIDGE model...")

            endog_glm_array = mod_dsn.loc[:, self.endog_glm].values

            # Get predictions (using standardized predictors if that's what the model expects)
            exog_glm_array: np.ndarray = mod_dsn.loc[:, self.exog_glm].values
            predictions: np.ndarray = model_results.predict(exog_glm_array)

            self.logger.info("Also check for the shapes of endog_glm_array and predictions")
            self.logger.info(f"Shape of endog_glm_array: {endog_glm_array.shape}")
            self.logger.info(f"Shape of predictions: {predictions.shape}")
            residuals: np.ndarray = endog_glm_array - predictions
    ...
    def performance_plots(
        ...
        resid : Union[np.ndarray, pd.Series]
            Pre-calculated residuals. If None, tries to access model_results.resid.
        ...
        # Calculate residuals
        indf = indf.copy()
        self.logger.info("Find class of residuals with model type {}".format(model_type))
        self.logger.info(type(resid))

        if resid is not None:
            indf['Residuals'] = resid
        else:
        ...
        self.logger.info(f"Saved {model_type} mean residuals plot for {performance_var} to {plot_filepath}")
        plt.cla() # clear the current axes
        plt.clf() # clear the current figure
        plt.close() # close the figure window
    ...
    def scatter_resid_with_predictors(
        ...
        # Save plot to file
        plot_filename = f"{model_type}_residuals_vs_{predictor_col}.png"
        plot_filepath = os.path.join(ModelTrain.plot_dir, plot_filename)
        plt.savefig(plot_filepath, dpi=300)
        self.logger.info(f"Saved {model_type} residuals scatter plot for {predictor_col} to {plot_filepath}")
        plt.cla() # clear the current axes
        plt.clf() # clear the current figure
        plt.close() # close the figure window

    def ols_comprehensive_diagnostics(self, cv_folds: int = 5):
        """
        Run comprehensive OLS diagnostics with cross-validation.
        """
        # Basic OLS with CV
...
if __name__ == "__main__":
    ...
    try:
        ...
        # set up code for OLS with CV and with comprehensive diagnostics
