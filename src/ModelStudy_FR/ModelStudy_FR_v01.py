...
class PreProcessor():
    ...
    def __init__(self, filePath: str, fileName: str, plot_dir: str):
        ...
        revenue_data: DataFrame = self.customer_data[revenue_columns] # type: ignore
    ...
    def log_transform_column(self, indf: pd.DataFrame, column_name: str) -> pd.Series:
    ...
    def map_ordinal_variables_and_category_order(self) -> Dict[str, Dict[str, int]]:
    ...
    def one_hot_encode_nominal_predictors(self, indf: pd.DataFrame) -> pd.DataFrame:
    ...
    def map_ordinal_predictors(self, indf: pd.DataFrame) -> pd.DataFrame:
        ...
        for map_col in ordinal_mappings.keys():
            ...
            # Use Series.map to convert categories -> codes. Keep originals if unmapped.
            outdf[map_col] = outdf[map_col].map(mapping)
        return outdf
    ...
    def log_transform_df(self, indf: pd.DataFrame) -> pd.DataFrame:
        ...
        for col in outdf_cols:
            if col in self.get_usage_metrics_columns() + self.get_revenue_after_discounts_columns():
                if pd.api.types.is_numeric_dtype(outdf[col]):
                    outdf[f"{col}_logged"] = self.log_transform_column(outdf, col)
                else:
                    print(
                        f"Predictor column '{col}' is not numeric. Skipping log transformation.")
                    print("\t", outdf[col].head())
            else:
                print(
                    f"Predictor column '{col}' is not in usage metrics or revenue after discounts columns. Skipping log transformation.")
    ...
    def log_transform_outcome(self, indf: pd.DataFrame) -> pd.DataFrame:
        ...
        if pd.api.types.is_numeric_dtype(outdf[self.outcome_col]):
            print()
            print(f"Running module: log_transform_outcome on column: {self.outcome_col}")
